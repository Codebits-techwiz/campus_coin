...
export default function Sitemap() {
  return (
    <div className="animate-fade-in py-16 bg-cc-mint-soft min-h-[70vh]">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-2">
          <Map className="w-7 h-7 text-cc-lime" />
          <h1 className="text-3xl font-extrabold text-cc-forest">Application Sitemap</h1>
        </div>
        <p className="text-cc-muted mb-10 max-w-2xl">
          Complete navigation map of Campus Coin as required by the SRS. Use the demo persona bar to jump between Public, Student, and Admin experiences.
        </p>
        <div className="grid sm:grid-cols-2 gap-6">
          {sections.map((sec) => (
            <div key={sec.title} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
              <h2 className="font-bold text-cc-forest text-lg mb-4 pb-2 border-b border-cc-mint">{sec.title}</h2>
              <ul className="space-y-2.5">
                {sec.links.map((l) => (
                  <li key={l.to}>
                    <Link to={l.to} className="text-sm text-cc-muted hover:text-cc-lime font-medium transition flex items-center gap-2">
